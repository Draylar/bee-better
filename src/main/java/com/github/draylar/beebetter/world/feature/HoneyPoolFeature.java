package com.github.draylar.beebetter.world.feature;
...
public class HoneyPoolFeature extends LakeFeature {
    ...
    public HoneyPoolFeature(Codec<SingleStateFeatureConfig> codec) {
        super(codec);
    }

    @Override
    public boolean generate(StructureWorldAccess world, ChunkGenerator chunkGenerator, Random random, BlockPos blockPos, SingleStateFeatureConfig singleStateFeatureConfig) {
        ...
        if (blockPos.getY() <= 4) {
            ...
            if (!world.getChunk(chunkPos.x, chunkPos.z, ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences(StructureFeature.VILLAGE).isEmpty()) {
                return false;
            } else {
