...
public class FindModdedHiveGoal extends NotAngryGoal {
    ...
    private Stream<BlockPos> method_23742(int i) {
        BlockPos blockPos = new BlockPos(this.getBee().getPos());
        if (this.getBee().world instanceof ServerWorld) {
            Stream<PointOfInterest> stream = ((ServerWorld) this.getBee().world).getPointOfInterestStorage().getInCircle((pointOfInterestType) ->
